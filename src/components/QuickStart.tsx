...
export function QuickStart() {
  return (
    <section aria-labelledby="quick-start-heading" className="bg-mist">
        ...
        <ul className="mt-8 grid gap-4 md:grid-cols-2">
          {links.map((item) => (
            <li key={item.href}>
              <a
                href={item.href}
                className="flex min-h-16 items-center rounded-2xl border-2 border-navy/15 bg-white px-5 text-lg font-semibold text-navy"
              >
                {item.label}
              </a>
            </li>
